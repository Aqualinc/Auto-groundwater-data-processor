...
#DataDirectory <- "H:\\WL Projects\\WL20023_CCC APP\\Data_Winter_2020"
DataDirectory <- "D:\\Projects\\Aqualinc\\projects\\APP\\Data\\Data Harvest 202009"
...
#OutputDataDirectory <- "H:\\WL Projects\\WL20023_CCC APP\\Data_Winter_2020\\BaroAndOffsetCorrected"
OutputDataDirectory <- "D:\\Projects\\Aqualinc\\projects\\APP\\Data\\Data Harvest 202009\\BaroAndOffsetCorrected"
...
```{r}
LoggerDataDirectory <- "H:\\WL Projects\\WL20023_CCC APP\\AllData"
...
DippedDataAtEndFile <- "D:\\Projects\\Aqualinc\\projects\\APP\\Auto-groundwater-data-processor\\Location-ordered July2020 APP Download Fieldsheet.xlsx" #Copy on Tim's laptop
...
DippedDataAtStartFile <- "D:\\Projects\\Aqualinc\\projects\\APP\\Auto-groundwater-data-processor\\20200221.Piezometer Downloads_TIM K.xlsx"     #On Tims Laptop
...
{
DippedStartData <- read_xlsx(DippedDataAtStartFile, sheet = "TIM K", range= cell_cols("B:L"))
...
{
  DippedEndData <- read_xlsx(DippedDataAtEndFile, sheet = "Field use", range= cell_cols("A:L"),col_types = c("numeric",rep("skip",8),"date","text","numeric"))
  
  #Remove any lines without a Date, assumed that they were not downloaded
  DippedEndData <- DippedEndData[!is.na(DippedEndData$Date),]
  ...
  DippedEndData$DateTimes <- DippedEndData$Date + as.numeric(DippedEndData$'Time') * 3600 * 24
...
AllDownloadedSites <- list.files(path = c("D:\\Projects\\Aqualinc\\projects\\APP\\Data\\Data Harvest 202009\\BaroAndOffsetCorrected"), pattern = 'APP\\d+_\\d{8}.*\\.csv$', recursive = TRUE, full.names = FALSE)
AllDownloadedSiteAPPNumbers <- as.numeric(sub(pattern="^APP([0-9]*)_.*csv","\\1",x=AllDownloadedSites))
FieldNoteSites <- DippedEndData$`APP No.`[which(!is.na(DippedEndData$Time))]
...
#Work through each APP number in turn, get the dipped data and the time of measurement and get the associated logged data, then compare the two
 ...
 DippedQC <- lapply(seq_along(AllDownloadedSiteAPPNumbers), function(Index) {   #for testing Index <- 82
  ...
  #Get the end dips
  DippedEndDepth <- DippedEndData$`Dipped Water Level (m) Winter 2020`[DippedEndDataIndex]
  ...
  #Get the start dips
    DippedStartDepth <- as.numeric(DippedStartData$`Dipped water level 2020 (m)`[as.numeric(DippedStartData$`APP No.`)== APPNo])
...
write.table(DipToAutoComparisonTable,"H:\\WL Projects\\WL20023_CCC APP\\Data_Winter_2020\\Master Spreadsheet\\DippedQC.csv",quote=FALSE,row.names = FALSE,sep=",")
```
...
```{r}
AllDataDirectory <- "H:\\WL Projects\\WL20023_CCC APP\\AllData"
...
write.table(StepChangeOnDownloadQCTable,"H:\\WL Projects\\WL20023_CCC APP\\Data_Winter_2020\\Master Spreadsheet\\StepChangeOnDownloadQC.csv",quote=FALSE,row.names = FALSE,sep=",")
```
