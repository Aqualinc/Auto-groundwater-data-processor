...
Times3[which(is.na(as.numeric(Times)))] <-
...
Times5 <- as.POSIXct(3600 * as.numeric(Times3), origin = "1970-01-01", tz="Etc/GMT+12")

#And add them to the dates
DippedData$DateTimes <- DippedData$Date + Times4 * 3600 * 24
...
#DippedQC <- lapply(seq_along(DippedData$`APP No.`), function(Index) {   #Index = 2
  DippedQC <- lapply(seq_along(DippedData$`APP No.`[1:15]), function(Index) {   #Index = 3
