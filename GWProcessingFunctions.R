...
#' @value A timeseries object of corrected groundwater levels 
...
BarometricCorrection <- function(GWSeries, AirPressureSeries) {
  ...
  if (GWRange[1] < BaroRange[1] | GWRange[2] > BaroRange[2]) stop('groundwater time series is not wholly within the range of the air pressure time series')
  ...
  #Take the gw time series date-times and add it to the barometric series.
  #Any duplicates, remove the ones without barometric pressure
  #use the na.approx to fill the remainder.
  AllignedAirPressure <- sapply(index(GWSeries), function(SingleDate) {
    #sample the date of choice from the barometric pressure and interpolate
...
BaroFile <- file.path(DataDirectory,"221_10_03_2017.xle")
